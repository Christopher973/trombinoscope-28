...
export const resolvers = {
  Query: {
    ...
    department: (_, { id }) =>
      prisma.department.findUnique({ where: { id: parseInt(id) } }),
    locations: () => prisma.location.findMany(),
    location: (_, { id }) =>
      prisma.location.findUnique({ where: { id: parseInt(id) } }),
  },
  Mutation: {
    createMember: (_, args) => {
    ...
    deleteDepartment: (_, { id }) =>
      prisma.department.delete({ where: { id: parseInt(id) } }),
    ...
    deleteLocation: (_, { id }) =>
      prisma.location.delete({ where: { id: parseInt(id) } }),
  },
  Member: {
    manager: (parent) =>
      parent.managerId
